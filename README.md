...
Les données, photos incluses, sont chiffrées dans le navigateur avec AES-GCM avant envoi. Le bucket reçoit une opération chiffrée et immuable par plante ou profil modifié, sous une empreinte de la clé. Le navigateur relit l'historique pour reconstruire la même vue sur tous les appareils ; l'ancienne sauvegarde est lue comme point de départ pour migrer sans perdre les plantes existantes. La clé reste dans le stockage local de chaque navigateur et est envoyée uniquement à la fonction du même site via HTTPS. Une opération est limitée à 4 Mo. Les modifications partent automatiquement après chaque édition ; les autres appareils actualisent à l'ouverture, au retour sur l'onglet et toutes les 20 secondes tant que l'onglet est visible. Deux appareils peuvent modifier différentes plantes en parallèle. Pour la même plante, la dernière opération reçue par R2 l'emporte. Les changements en attente restent locaux pendant une coupure réseau et repartent à la reconnexion. Les onglets ouverts avant cette mise à jour doivent être rechargés pour utiliser le nouveau protocole.
